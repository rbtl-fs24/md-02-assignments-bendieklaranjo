...
gapminder_2007 <-  gapminder_summary_2007  |> 
  group_by(continent) |> 
  summarise(
    count = n(),
...
5.  What are the data types of the three variables used for aesthetic mappings?
    ...
    *continent: non-numerical, categorical variable,*

    *lifeExp = numerical discrete variable,*
...
-   *`geom_bar()` requires a single variable `aes(x = )`that should be a categorical variable and then counts up the number of observations (rows) for this variable and plots them as bars.*

-   *`geom_col()` requires two variables `aes(x = , y = ),x` is categorical, `y` is continuous (numeric).*

-   *Since in the last exercise in the geom_col() function the `y` variable was set to `count` , geom_col() does the exact same function as geom_bar(). That is why the plots are identical.*

    *Source: `(https://argoshare.is.ed.ac.uk/healthyr_book/bar-plots.html)`*
...
ggplot ( data = gapminder_2007,
         mapping = (aes(x = lifeExp,
                        fill = continent))) +
  geom_histogram(col = "grey30", breaks = seq(40,58,2.5)
  )
